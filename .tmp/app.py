...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def copySelectedShape(self):
        ...
        for shape in added_shapes:
            if self.canvas.shapeIsLocked(shape):
                self.setGroundTruthDictAttrGT(s)
    ...
    def setGroundTruthDictAttrGT(self, shape_dict):
        self.setGroundTruthDictAttr(self, shape_dict, True, None):
    
    def setGroundTruthDictAttrNonGT(self, shape_dict, sourceFolder):
        self.setGroundTruthDictAttr(self, shape_dict, False, sourceFolder):
    
    def setGroundTruthDictAttr(self, shape_dict, isGroundTruth, sourceFolder):
        if isGroundTruth:
            shape_dict['source'] = self.groundTruthDirName
            shape_dict['opacity'] = self.groundTruthOpacity
            shape_dict['locked'] = False
            shape_dict['disp_label'] = self.getShapeDisplayLabel(shape_dict)
        else:
            pass
            ...
            for filename_addl in addl_files:
                ...
                if labelFile_addl:
                    for s in labelFile_addl.shapes:
                        self.setGroundTruthDictAttrNonGT(s, False, self.dfAllImages['Image Folder'].loc[filename_addl])
                        shapes.append(s)
